  ...
  useEffect(() => {
    ...
    chartInstance.current = new Chart(chartRef.current.getContext("2d"), {
      ...
      data: {
        labels,
        datasets: [
          { label: "Estoque atual", data: atual, backgroundColor: CHART_COLORS.accent, borderRadius: 6, maxBarThickness: 26 },
          ...
          <div className="card section">
            ...
            {itensMaisConsumidos.length === 0 ? <div className="empty-state">Sem dados para os filtros.</div> : (
              <ChartBox
                type="bar"
                data={{
                  labels: itensMaisConsumidos.map((i) => (i.nome || i.codigo).slice(0, 22)),
                  datasets: [{ label: "Consumido", data: itensMaisConsumidos.map((i) => i.valor), backgroundColor: CHART_COLORS.accentSoft, borderRadius: 6, maxBarThickness: 30 }],
                }}
                options={baseOptions}
              />
            )}
